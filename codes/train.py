...
from model_unet import SRF_UNet, Res_UNet, Swin_UNet, UNet
...
opt_gamma = 0.5
seg_lr = 1e-4
data_dir = "F:\datasets\octa500\OCTA-500"
thred_val = 0.7
other_info = "SRF-Dice" # "noCenter"
...
for fov in ["3M", "6M"]:
    time_str = "-".join(["{:0>2}".format(x) for x in time.localtime(time.time())][:-3] + [fov, other_info])
    result_dir = "./results/" + time_str
    os.mkdir(result_dir)

    octa_dataset = octa500_Dataset(data_dir=data_dir, fov=fov)
    ...
    for fold_i in range(k_fold):
        ...
        model = SRF_UNet(img_ch=2).to(device)
        # model = Swin_UNet(img_ch=2).to(device)
        # model = Res_UNet(img_ch=2).to(device)
        ...
        for epoch in tqdm(range(Epoches)):
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                loss_vessel = DiceLoss()(pred_vessel, label_vessel)
                ...
                loss_seg = 0.5 * loss_vessel + 0.5 * loss_faz + 0.2 * loss_heatmap
                loss_seg.backward()
                optimizer.step()
            ...
            if epoch % check_interval == 0:
                ...
                for i, data in enumerate(val_loader, 0):
                    ...
                    metrics_dct["loss_seg"] = 0.5 * metrics_dct["loss_vessel"] \
                                              + 0.5 * metrics_dct["loss_faz"] + 0.2 * metrics_dct["loss_heatmap"]
                ...
                if float(metrics_test_dct["loss_seg"][-1]) < best_loss:
                    best_loss = float(metrics_test_dct["loss_seg"][-1])
                    if not os.path.exists("weights"):
                        os.mkdir("weights")
                    torch.save(model.state_dict(), 'weights/model_weights_{}_{}.pth'.format(fov, other_info))
