...
class Swin_UNet(nn.Module):
    def __init__(self, img_ch=3, output_ch=1):
        ...
        for ch_in_1, ch_in_2, ch_out_1, ch_out_2 in zip([2048, 1024, 512, 256, 64], [2048, 1024, 512, 64, 64],
                                                        [1024, 512, 256, 64, 64], [1024, 512, 256, 64, 32]):
            self.decoder_vessel.append(up_conv(ch_in=ch_in_1, ch_out=ch_out_1))
            self.decoder_vessel.append(res_conv_block(ch_in=ch_in_2, ch_out=ch_out_2))
    ...
    def decoder(self, D, x1, x2, x3, x4, down_pad, right_pad):
        # decoding + concat path
        d5 = D[0](x4.permute(0, 3, 1, 2))
        d5 = torch.cat((x3.permute(0, 3, 1, 2), d5), dim=1)
        ...
        d4 = D[2](d5)
        d4 = torch.cat((x2.permute(0, 3, 1, 2), d4), dim=1)
        d4 = D[3](d4)

        d3 = D[4](d4)
        d3 = torch.cat((x1.permute(0, 3, 1, 2), d3), dim=1)
        d3 = D[5](d3)
    ...
    def forward(self, x):
        ...
        down_pad = False
        right_pad = False
        if y3.permute(0, 3, 1, 2).size()[2] % 2 == 1:
            y3 = y3.permute(0, 3, 1, 2)
            y3 = F.pad(y3, (0, 0, 0, 1))
            y3 = y3.permute(0, 2, 3, 1)
            down_pad = True
        if y3.permute(0, 3, 1, 2).size()[3] % 2 == 1:
            y3 = y3.permute(0, 3, 1, 2)
            y3 = F.pad(y3, (0, 1, 0, 0))
            y3 = y3.permute(0, 2, 3, 1)
            right_pad = True
        ...
        x_vessel, x_faz, x_heatmap = self.decoder(self.decoder_vessel, y1, y2, y3, y4, down_pad, right_pad), \
                                     self.decoder(self.decoder_faz, y1, y2, y3, y4, down_pad, right_pad), \
                                     self.decoder(self.decoder_heatmap, y1, y2, y3, y4, down_pad, right_pad)

        return x_vessel, x_faz, x_heatmap, torch.tensor([1, 0, 0, 0], dtype=torch.float)


class UNet(nn.Module):
    def __init__(self, img_ch=2, n_classes=2, bilinear=False):
    ...
    def forward(self, x):
        ...
        logits = self.outc(x)
        # return logits
        x_vessel, x_faz, x_heatmap = torch.split(logits, 1, dim=1)
        return x_vessel, x_faz, x_heatmap, torch.tensor([1, 0, 0, 0], dtype=torch.float)
