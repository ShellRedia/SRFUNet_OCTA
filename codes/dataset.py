...
class octa500_Dataset(Dataset):
    def __init__(self, data_dir = "F:\datasets\octa500\OCTA-500", fov="3M"):
        image_dir_OPL_BM = r"{}\OCTA_{}\Projection Maps\OCTA(OPL_BM)".format(data_dir, fov)
        image_dir_ILM_OPL = r"{}\OCTA_{}\Projection Maps\OCTA(ILM_OPL)".format(data_dir, fov)

        seg_label_dir = r"{}\OCTA_{}\GroundTruth".format(data_dir, fov)
        landmark_dir = r"{}\OCTA_{}\Keypoints".format(data_dir, fov)
        disease_label_excel_path = r"{}\OCTA_{}\TextLabels.xlsx".format(data_dir, fov)
